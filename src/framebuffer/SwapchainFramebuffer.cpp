...
SwapchainFramebuffer::SwapchainFramebuffer(
    const vk::Image &image, const vk::Format image_format,
    ...
    : swapchain_manager(swapchain_manager),
      color_image(swapchain_manager->getDimensions(), image, image_format,
                  device_manager),
    ...
    this->framebuffer = framebufferUtil::createBasicFramebuffer(
        this->color_image, this->depth_image, render_pass_registry,
...
std::unique_ptr<IFramebuffer> SwapchainFramebuffer::createSwapchainFramebuffer(
    ...
    return std::unique_ptr<IFramebuffer>(
        new SwapchainFramebuffer(image, image_format, swapchain_manager,
                                 device_manager, render_pass_registry));
}
...
bool SwapchainFramebuffer::hasColorImage() const { return true; }

const IImage &SwapchainFramebuffer::getColorImage() const {
    return this->color_image;
}

bool SwapchainFramebuffer::hasDepthImage() const { return true; }
