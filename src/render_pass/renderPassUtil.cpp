...
vk::AttachmentDescription getAttachmentDescription(bool clear_on_load,
    ...
    desc.setLoadOp(clear_on_load ? vk::AttachmentLoadOp::eClear
...
getColorAttachmentDescription(bool clear_on_load, bool finalize_as_texture) {
    return getAttachmentDescription(
        clear_on_load,
        clear_on_load ? vk::ImageLayout::eUndefined
                      : vk::ImageLayout::ePresentSrcKHR,
        finalize_as_texture ? vk::ImageLayout::eShaderReadOnlyOptimal
                            : vk::ImageLayout::ePresentSrcKHR,
        imageUtil::DEFAULT_FRAMEBUFFER_COLOR_IMAGE_FORMAT);
}

vk::AttachmentDescription
getDepthAttachmentDescription(bool clear_on_load, bool finalize_as_texture) {
    return getAttachmentDescription(
        clear_on_load,
        clear_on_load ? vk::ImageLayout::eUndefined
...
getColorAttachmentDescriptions(const std::vector<bool> &clears,
    ...
    for (uint32_t i = 0; i < descriptions.size(); i++) {
        descriptions[i] =
            getColorAttachmentDescription(clears[i], finalize_as_texture);
    }
...
getColorAttachmentReferences(uint32_t num_references,
                             uint32_t binding_offset = 0) {
    std::vector<vk::AttachmentReference> references(num_references);

    for (uint32_t i = 0; i < num_references; i++) {
        references[i] = vk::AttachmentReference(
            binding_offset + i, vk::ImageLayout::eColorAttachmentOptimal);
    }
...
vk::AttachmentReference getDepthAttachmentReference(uint32_t binding_offset) {
    return vk::AttachmentReference(
        binding_offset, vk::ImageLayout::eDepthStencilAttachmentOptimal);
}
...
vk::RenderPass
createRenderPass(const std::vector<vk::AttachmentDescription> &descriptions,
...
bool RenderPassSetup::getDepthClear() const {
    if (!this->depth_clear.has_value()) {
        throw std::runtime_error("[RenderPassSetup] Tried to get depth clear "
                                 "on render pass setup without depth");
    }
...
RenderPassSetup::setNumColorAttachments(uint32_t num_color_attachments) {
    std::vector<bool> new_color_clears =
        std::vector<bool>(num_color_attachments, true);
    for (uint32_t i = 0;
         i < std::min(this->color_clears.size(), new_color_clears.size());
         i++) {
...
vk::RenderPass createRenderPass(const RenderPassSetup &setup,
                                const vk::Device &device) {
    ...
    if (setup.hasDepthAttachment()) {
        attachment_descriptions.push_back(getDepthAttachmentDescription(
            setup.getDepthClear(), setup.getFinalizeAsTexture()));
        depth_reference = getDepthAttachmentReference(color_references.size());
    }

    vk::SubpassDescription subpass_description =
        createSubpassDescription(color_references, depth_reference);

    return createRenderPass(attachment_descriptions, subpass_description,
                            device);
}
}; // namespace wg::internal::renderPassUtil
